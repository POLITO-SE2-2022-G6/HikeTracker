...
import { createPoint, editPoint, pointById, pointsList, fullList } from "../DAO/pointDao";
...
//Get points from hike id
hRouter.get("/hike/:id", checkSchema({
    id: {
        in: ['params'],
        isInt: true
    }
    ...
    if (!validationResult(req).isEmpty()) return res.status(400).json({ errors: "Illegal Data" });
    const id = parseInt(req.params.id);
    const hike = await pointsList(id);
    if (!hike) return res.status(404).json({ error: "Hike not found" });
    res.send(hike);
});
...
//Get all points
hRouter.get("", isGuide, async (req: express.Request, res: express.Response) => {
    const hike = await fullList();
    res.send(hike);
});
